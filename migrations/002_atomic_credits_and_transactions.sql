...
-- 2. Index for idempotent request lookup
CREATE INDEX IF NOT EXISTS idx_credit_transactions_user_req ON public.credit_transactions (user_id, request_id);
...
-- 4. New User Trigger: Awards exactly 50 credits once (NO tier column reference)
...
-- 6. Atomic Reservation RPC: reserve_credits
...
BEGIN
    ...
    -- If profile missing, auto-provision with 50 credits
    ...
    VALUES (gen_random_uuid(), p_user_id, -p_amount, 'feature_usage', p_feature, p_request_id, 'pending', NOW());
...
-- 7. Atomic Settlement RPC: settle_credits
...
BEGIN
    ...
    WHERE user_id = p_user_id AND request_id = p_request_id AND status = 'pending';

    RETURN json_build_object('success', true);
END;
$$;

-- 8. Atomic Release RPC: release_credits (Internal backend usage only on AI provider failure)
...
-- Deduct credits wrapper calling reserve_credits for compatibility
...
AS $$
BEGIN
    RETURN public.reserve_credits(p_user_id, p_amount, p_feature, p_request_id);
END;
$$;
