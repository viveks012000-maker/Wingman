...
const replacement = `// 2. Configure Locked CORS Policy
...
// an old environment variable still contains them; this prevents a stale deployment setting
...
function isOriginAllowed(origin, allowedList) {
    ...
    // Wildcard origins are supported only for explicitly non-production development use.
    if (!IS_PROD) {
...
assert.ok(server.includes("if (!IS_PROD) {\n        for (const item of allowedList)"), 'Wildcard origin matching must be development-only');
