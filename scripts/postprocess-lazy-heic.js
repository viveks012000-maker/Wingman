...
const REAL_RUNTIME = path.join(OUT, 'vendor', 'heic2any.min.js');
...
if (eagerMatches.length !== 1) {
    fail(`Expected exactly one eager HEIC runtime tag, found ${eagerMatches.length}`);
}
...
if (!html.includes('<script src="./vendor/heic2any-loader.js"></script>')) {
    fail('Lazy HEIC loader tag was not written to app.html.');
}
...
release.files['vendor/heic2any.min.js'] = sha256(REAL_RUNTIME);
...
if (rewrittenRelease.files['vendor/heic2any.min.js'] !== sha256(REAL_RUNTIME)) fail('HEIC runtime release hash is stale.');
