...
4. CASING: 100% strictly lowercase text. Max 1 natural emoji at the end (😏, 🙈, 😉, 🥹, 💅).`;
    ...
    window.loadUserCreditState = function(user) {
        try {
            ...
            const key = window.getUserKey("wingman_credits", userId);
            const stored = safeStorage.get(key);
            state.credits = (stored !== null && !isNaN(parseInt(stored))) ? parseInt(stored) : 0;
            syncCredits();
        } catch (e) {}
    };
    ...
    // Server‑sync credit check – always pulls latest from backend
    window.checkCreditBalance = async function () {
        try {
            ...
            const token = (window.currentSupabaseSession ? window.currentSupabaseSession.access_token : null) || safeStorage.get("wingman_jwt_token");
            if (token) reqHeaders['Authorization'] = 'Bearer ' + token;

            const response = await fetch(apiBase + '/api/credits/sync', {
                method: 'GET',
                headers: reqHeaders,
                credentials: 'include',
            ...
            if (response.ok) {
                ...
                if (res) {
                    if (typeof res.credits === 'number') realCredits = res.credits;
                    else if (res.data && typeof res.data.credits_inr === 'number') realCredits = Math.round(res.data.credits_inr * 10);
                }
                if (realCredits !== null) {
                    window.updateUICredits(realCredits);
                    return realCredits;
                }
            }
        } catch (e) {}
    ...
    window.simulateDemoPurchase = async function (creditAmount, btnEl) {
        ...
        try {
            ...
            const selectedTierVal = (state.selectedTier && state.selectedTier.value) || 'credit_bundle';
            ...
            if (!response.ok) {
                ...
                const errMsg = errJson.error || "Purchase verification failed. Please try again.";
                window.showToast(errMsg, "warning");
                if (response.status === 401 && typeof window.openAuthRequiredModal === "function") {
            ...
            if (data && data.success && typeof data.credits === "number") {
                ...
                window.showToast("Successfully added " + (data.creditsAdded || creditAmount) + " credits to your account!", "success");
                if (typeof window.closePurchaseModal === 'function') window.closePurchaseModal();
            } else {
                window.showToast(data.error || "Failed to update credits.", "warning");
            }
        } catch (err) {
            console.error("Credits purchase error:", err.message);
            window.showToast("Network error during credit purchase.", "warning");
        } finally {
            if (btnEl) {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (!response.ok) {
                    ...
                    const errText = errJson.error || "Insufficient credits. Current balance: " + (state.credits || 0) + " credits. Please top up.";
                    if (response.status === 402 && typeof window.showToast === "function") {
                        window.showToast("Insufficient credits. Current balance: " + (errJson.credits || state.credits || 0) + " credits.", "error");
                    }
        ...
        try {
            ...
            const promptPayload = {
                ...
                messages: [
                    {
                        role: "system",
                        content: "Generate 10 copy-pasteable optimized bio templates based on style: " + styleInstruction + ". Strict format: output [BIO_OPTION_1] content and repeat for options up to 10."
                    },
                    {
                        role: "user",
                        content: raw
            ...
            if (aiText) {
                ...
                window.showToast("Profile Audit Complete! (10 Credits Processed)", "success");
            } else {
                if (emp) emp.classList.remove("hidden");
                if (skel) skel.classList.add("hidden");
            }
            ...
            stopTelemetryTracker("optimize", "optimize-telemetry-status", "optimize-telemetry-pct", "optimize-telemetry-bar", "AUDIT COMPLETE");
            state.isLoading = false;
    ...
    window.loadPresetBio = function (btn) {
        ...
        const bi = $("bioInput");
        if (bi && btn.getAttribute("data-sample")) {
    ...
    // ============================================================
    // PRACTICE PARTNER CHAT DRILL PROTOCOLS
    // ============================================================
    window.clearAndResetChatbox = function() {
        ...
        // Hard Reset State & Session Metrics
    ...
    // ============================================================
    // TURN EVALUATION CARD & RETRY LOGIC
    // ============================================================
    ...
    // ============================================================
    // DATING FLIGHT SIMULATOR ENGINE
    // ============================================================
    window.currentAttractionScore = 65;
    window.sessionStats = {
        messagesSent: 0,
    ...
    window.updateCharacterMood = function(moodStr) {
        // Mood indicator removed per spec
    };

    window.renderInFeedCheckpointCard = function(checkpointData, turnIndex) {
        // In-feed analysis cards removed per spec
    };
