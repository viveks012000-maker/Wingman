...
async function verifyLoaderBehavior() {
    {
        ...
        assert.strictEqual(injected.src, './vendor/heic2any.min.js');
...
function verifyFinalArtifact() {
    ...
    try {
        ...
        assert.ok(fs.existsSync(path.join(OUT, 'vendor', 'heic2any.min.js')), 'real HEIC runtime must remain available for first HEIC use');
        ...
        assert.strictEqual(release.files['vendor/heic2any.min.js'], sha256(path.join(OUT, 'vendor', 'heic2any.min.js')));
    } finally {
        fs.rmSync(OUT, { recursive: true, force: true });
    }
}
