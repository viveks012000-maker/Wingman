...
const EXPECTED_SHA = 'ab731892405a9b4fff0e227aff7539ff04f81f90';
...
async function probeCloudflare() {
    ...
    return { release: release.sourceCommit, appHtmlCanonicalStatus: appHtmlRoute.status };
}
...
async function probeRailway() {
    ...
    const allowedGet = await request(`${RAILWAY}/api/config`, {
    ...
    return { health: healthBody.database };
}
...
(async () => {
    ...
    console.log(JSON.stringify({ expectedSha: EXPECTED_SHA, cloudflare, railway }, null, 2));
})().catch(error => {
