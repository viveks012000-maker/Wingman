...
assert.ok(buildSource.includes('const appCsp = cspFor(false);'), 'Netlify dashboard CSP must use the strict eval-free policy (new HEIC runtime uses USE_UNSAFE_EVAL=0)');
assert.ok(!indexHtml.includes("'unsafe-eval'"), 'Landing-page meta CSP must remain eval-free');
assert.ok(!appHtml.includes("'unsafe-eval'"), 'Dashboard CSP must not contain unsafe-eval (new HEIC runtime uses USE_UNSAFE_EVAL=0)');
assert.ok(appHtml.includes('vendor/heic2any-loader.js'), 'Dashboard must load HEIC via lazy loader shim');
