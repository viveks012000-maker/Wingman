...
assert.strictEqual(serverFile.includes('// Exclusive Authoritative Source: Atomic Postgres RPC function'), false, 'Comments must be consistent');
...
// deduct_credits checks
assert.strictEqual(migrationSql.includes('FOR UPDATE'), true, 'deduct_credits must use FOR UPDATE row locking');
...
// refund_credits checks
assert.strictEqual(migrationSql.includes('SELECT ABS(amount) INTO v_deducted_amount'), true, 'refund_credits must verify original deduction amount');
...
console.log('✔ Test 4 Passed: SQL Migration 002 row-locking, refund-capping, 50-credit default, and idempotency verified');
...
// 7. TEST SCREENSHOT PAYLOAD LIMITS (38MB ROUTE BODY, 25MB DECODED)
...
console.log('✔ Test 7 Passed: Screenshot payload size limits aligned with 5x5MB decoded capacity');

// 8. TEST LOCALSTORAGE BASE64 REMOVAL IN app.js
